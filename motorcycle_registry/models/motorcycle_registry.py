...
class MotorcycleRegistry(models.Model):
    ...
    _sql_constraints = [('vin_unique', 'UNIQUE(vin)', 'Odoopsie! Another registration for this VIN number already exists.')]

    registry_number = fields.Char(string='Registry Number', copy=False, required=True, readonly=True, default='New')
    ...
    certificate_title_fname = fields.Char(string='Certificate of Title File Name')
    ...
    year = fields.Char(compute='_compute_from_vin', store=True) 
    ...
    def _check_license_plate_size(self):
        ...
        for registry in self.filtered(lambda r: r.license_plate):
            if registry.license_plate:
                ...
                if not match:
                    raise ValidationError('Odoopsie!Only Invalid License Plate KAI123LN')
    ...
    def _check_vin_pattern(self):
        ...
        for registry in self.filtered(lambda r: r.vin):
            ...
            if not registry.vin[0:2] == 'KA':
                raise ValidationError('Odoopsie!Only Motorcycles from Kawiil Motors are allowed')
